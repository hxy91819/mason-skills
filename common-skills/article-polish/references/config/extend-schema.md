...
style: natural  # natural | concise | vivid | formal | conversational | academic | storytelling | elegant | or custom string
...
| `style` | string | `natural` | Writing style (`natural` / `concise` / `vivid` / `formal` / `conversational` / `academic` / `storytelling` / `elegant` / custom) |
