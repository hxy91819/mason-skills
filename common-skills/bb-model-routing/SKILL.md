...
`--kind oracle` 会由脚本要求专家亲自调查和回答，并加入禁止递归转交咨询及证据不足时返回父 Agent 的边界；它可使用当前权限提供的命令与工具，不额外限制分析手段。其他类型原样传递任务。`--dry-run` 可检查实际发送的完整 prompt。
...
`defaults.<路由>` 可配单个别名或有序别名链。链是同档 fallback：每个候选在自己的 `routes` 中按本次路由项解析（simple 失败仍只取下一候选的 simple/default），脚本按序校验 provider 可用性、权限与模型目录，首个通过者派发。`--agent` 是固定单候选，不进链。候选全部不可用时汇总各候选错误报告；spawn 发出后不重试，确认线程未创建后用 `--fallback-from <链中别名>` 续派。

权限须符合任务授权；配置缺失或校验失败时说明缺口，不静默换模型或升权。

派发标题由脚本统一加 `[Agent]` 前缀；需要简短标题时传 `--title`，省略时从任务文本截取。

根据返回结果报告线程 ID、实际选择和状态；发生 fallback 时说明链上失败原因与实际候选。创建成功不代表任务完成；创建结果不明时先查询线程，避免重复派发。
...
并发写入可能产生修改冲突时，用 `use-worktree` 隔离；无冲突时沿用当前环境。操作前检查分支、工作区和 worktree，遵守用户授权。隔离 worktree 准备好后可把目录路径传给 `--environment`，由 BB 创建附着环境。
...
使用 `--dry-run` 只读检查当前选择、目标环境、启动参数与剩余 fallback 链；派发返回实际选择、`attempts` 链上失败记录、`fallbacks` 未尝试候选与原始创建回执。当前没有持久运行历史、裁决回写或跨运行聚合，无法统计长期成功率；故障定位依赖返回错误和 BB 线程记录（`bb thread show <线程ID>` 回看）。
