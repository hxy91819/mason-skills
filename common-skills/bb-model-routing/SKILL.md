...
从 `simple` 开始，只在出现明确升级信号时提高难度：

- `simple`：目标、实现路径和验收边界清楚，可在一个聚焦上下文内闭环。
- `medium`：需要协调多个模块、比较真实方案，或解决会影响实现方向的不确定性。
- `complex`：根因尚未定位、需要跨系统设计，或属于高影响且难以回退的技术裁决。

文件数量、测试数量、任务描述篇幅和“需要完整验证”本身不构成升级理由。能由 simple 候选按明确步骤完成并验证的任务保持 simple；不要为了保险选择更高档位。
