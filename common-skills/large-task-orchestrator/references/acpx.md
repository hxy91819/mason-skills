...
Shell wrappers and aliases such as `codexl` or `codexp` are not ACPX agents merely because they launch a coding CLI. Route Codex work through `acpx codex`. Add a custom `agents` entry or use `--agent` only when its command exposes an ACP-compatible stdio server; an ordinary interactive CLI wrapper does not qualify.
