...
- `max_difficulty`: optional difficulty ceiling for this candidate. The difficulty ladder is `routine < standard < complex < critical`. A candidate is eligible only when the Story's difficulty sits at or below its ceiling; a Story above the ceiling skips the candidate and continues with the next one.
