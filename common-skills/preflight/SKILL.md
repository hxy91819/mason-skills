...
开工前核查：spec 或当前指令所依赖的凭据、权限、工具和外部环境是否真正可用，把"worker 跑到一半才发现缺东西"的坑提前清掉。在 to-spec 之后、implement 之前跑最合适。上下文已有 spec 就核查 spec，否则以当前指令为目标；两者都构不成完整目标时，先请用户补充，不做臆测。
