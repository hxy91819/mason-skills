...
4. **落盘报告。** 产出三态清单表：依赖 / 用途 / 状态 / 证据或处置。spec'd 项同时汇总成“需要用户的支持”一节，逐项写明具体动作（申请哪个系统的什么权限或 token、提供什么凭据、授权哪次安装），让用户一次处理完，而不是 worker 跑起来后反复被打断。spec 可编辑就直接修订，否则作为 issue 评论或独立文件持久化——报告只留在聊天里等于没做，后续 worker 看不到。
