...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_claude_reviewer_defaults_inherit_user_claude_config(self) -> None:
        keys = [
            "AUTOREVIEW_MODEL",
            "AUTOREVIEW_THINKING",
        ...
        with self.helper["preserve_env"](keys):
            for key in keys:
            ...
            self.assertIsNone(claude.thinking)
            ...
            self.assertEqual(explicit.model, "glm-5.2:cloud[1m]")
            self.assertEqual(explicit.thinking, "max")

    def _observe_claude_command(self, args: argparse.Namespace) -> list[str]:
    ...
    def test_claude_default_omits_model_and_effort_flags(self) -> None:
        cmd = self._observe_claude_command(
            argparse.Namespace(
                claude_allowed_tools=None,
                claude_bin="claude",
                fallback_model=None,
    ...
    def test_claude_explicit_model_and_thinking_are_passed(self) -> None:
        cmd = self._observe_claude_command(
            argparse.Namespace(
                claude_allowed_tools=None,
                claude_bin="claude",
        ...
        self.assertEqual(cmd[cmd.index("--model") + 1], "glm-5.2:cloud[1m]")
