...
class AutoreviewPriorityTests(unittest.TestCase):
    ...
    def test_cli_max_priority_overrides_env(self) -> None:
        with mock.patch.dict(os.environ, {"AUTOREVIEW_MAX_PRIORITY": "P1"}):
            with mock.patch.object(sys, "argv", ["autoreview", "--max-priority", "P3"]):
                args = AUTOREVIEW.parse_args()
        self.assertEqual(args.max_priority, "P3")

    def test_priority_threshold_prompt_follows_max_priority(self) -> None:
        self.assertEqual(
            AUTOREVIEW.priority_threshold_prompt("P0"),
            "Finding threshold: report only P0. "
        ...
        self.assertIn("report only P0, P1", AUTOREVIEW.priority_threshold_prompt("P1"))
        self.assertIn("report only P0, P1, P2, P3", AUTOREVIEW.priority_threshold_prompt("P3"))

    def test_scope_policy_defers_to_finding_threshold(self) -> None:
        policy = AUTOREVIEW.review_scope_policy()
        self.assertIn("finding threshold below", policy)
        self.assertNotIn("must be fixed before this target can land", policy)
...
class AutoreviewCodexCooldownTests(unittest.TestCase):
    def test_usage_limit_message_is_detected(self) -> None:
        result = subprocess.CompletedProcess(
            ["codex"],
            1,
            "",
        ...
        self.assertTrue(AUTOREVIEW.codex_usage_limit_failure(result))
        capacity = subprocess.CompletedProcess(
            ["codex"],
            1,
            "",
            "gpt-5.6-sol is temporarily unavailable due to capacity",
        )
        self.assertFalse(AUTOREVIEW.codex_usage_limit_failure(capacity))

    def test_usage_limit_failure_records_cooldown_and_later_skips_codex(self) -> None:
        with tempfile.TemporaryDirectory(prefix="autoreview-codex-cooldown.") as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            repo.mkdir()
            state = root / "state"
            ...
            with mock.patch.dict(os.environ, {"AUTOREVIEW_STATE_DIR": str(state)}):
                with mock.patch.object(AUTOREVIEW, "resolve_command", return_value="/usr/bin/codex"), mock.patch.object(
                    ...
                    with self.assertRaisesRegex(SystemExit, "usage-limit cooldown recorded"):
                        AUTOREVIEW.run_codex(args, repo, "review")
                    with self.assertRaisesRegex(SystemExit, "usage-limit cooldown"):
                        AUTOREVIEW.run_codex(args, repo, "review")
                until = AUTOREVIEW.active_codex_cooldown(repo)
                self.assertIsNotNone(until)
                self.assertGreater(until, time.time() + 50 * 60)
                self.assertLess(until, time.time() + 70 * 60)

            self.assertEqual(heartbeat_calls["count"], 1)

    def test_apply_cooldown_switches_codex_only_review_to_claude(self) -> None:
        with tempfile.TemporaryDirectory(prefix="autoreview-codex-cooldown-apply.") as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            repo.mkdir()
            state = root / "state"
            with mock.patch.dict(os.environ, {"AUTOREVIEW_STATE_DIR": str(state)}):
                AUTOREVIEW.record_codex_cooldown(repo, now=time.time())
                args = AUTOREVIEW.reviewer_test_args(engine="codex", ignore_codex_cooldown=False)
                reviewers = AUTOREVIEW.apply_codex_usage_cooldown(
                    args, AUTOREVIEW.reviewer_args(args), repo
                )
            self.assertEqual([reviewer.engine for reviewer in reviewers], ["claude"])
            self.assertIsNone(reviewers[0].model)

    def test_ignore_codex_cooldown_keeps_codex_reviewer(self) -> None:
        with tempfile.TemporaryDirectory(prefix="autoreview-codex-cooldown-ignore.") as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            repo.mkdir()
            state = root / "state"
            with mock.patch.dict(os.environ, {"AUTOREVIEW_STATE_DIR": str(state)}):
                AUTOREVIEW.record_codex_cooldown(repo, now=time.time())
                args = AUTOREVIEW.reviewer_test_args(engine="codex", ignore_codex_cooldown=True)
                reviewers = AUTOREVIEW.apply_codex_usage_cooldown(
                    args, AUTOREVIEW.reviewer_args(args), repo
                )
