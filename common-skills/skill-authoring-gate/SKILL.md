...
审查 Skill diff 时，把这类新增要求单独列出，并指出对应的用户原话或既有权威政策。找不到明确依据时视为补偿性授权，不写入 Skill；若已在 diff 中则删除。用户只要求改进流程、提高安全性或审查 Skill，不等于授权增加人工审批。保留未要求调整的既有授权边界；外部系统或宿主本来强制的审批规则可以如实记录，但不得包装成用户偏好或借机扩大。
...
- 对每个新增的审批、确认、只读或人工停点要求，核对明确的用户授权或既有强制政策；没有依据则不得交付。
