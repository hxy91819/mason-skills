...
- 对明显空带，指出相邻的具体内容及所在区域，并比较同类页面的主体占用、字号和重心。连续内容页重复出现大块底部空带，应作为一组检查；封面或章节页的停顿理由只适用于承担该任务的页面。
- 保留空带时，说明它服务哪段内容、为什么需要当前尺度。用重排作为反证：若增大关键图文、改善行距或调整分布能提升投影阅读，又不损伤分组与主次，当前留白应列为改进项。单有“页脚缓冲”或“没有遮挡”的描述不足以说明构图合理；不设所有页面通用的留白像素合格线。
- 修后同时看容器和内容：放大卡片若只是把外部空带转移为卡内空洞，文字仍小且集中在顶部，问题仍未解决。
...
完成标准：每页结论说明阅读任务与主体布局是否匹配，不以“无溢出/无回归”代替构图判断；每个 flag 都在 `flagRulings` 里有具体画面依据；看图发现而脚本未报的问题也已记录。
...
- 全量重跑第 1–2 节，包括未直接改动的页；对照上一轮 flags 看变化，每条残留线索都要有视觉裁决。把本轮修法与同类未改页并置判断：同一种构图问题应采用一致尺度，不能只确认已报项变好。线索消失不代表修好（可能只是绕开了检测），线索残留也不代表没修好，以画面为准。
