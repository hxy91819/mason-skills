...
- **审查者裁决每条 flag**，结论三选一，写进 `findings.json` 的 `flagRulings`（`{round, id, page, ruling, reason}`）。脚本 ID 每次运行重新编号，以轮次与 ID 联合定位，不用本轮 `V12` 覆盖上一轮同号记录：
  - `valid`：画面上确实影响阅读 → 形成 finding，`why` 写观众看到的影响，不写"flag V12 触发"。
  - `dismissed`：看图后不成立 → 点明该页的对象、位置和设计用途，解释线索所指的尺寸或关系为何合理。相同布局可以共用判断原则，每页仍要对应自己的画面证据；“有意留白”“不影响阅读”“没有溢出”本身不足以驳回构图问题。留白判断按清单的「留白」节执行。
...
- **主 Agent 同样独立判断审查意见**：除已列 findings 外，复看用户指出的页面，以及被驳回线索中视觉影响较大、连续多页重复、或与已采纳问题同类的代表页。发现相同遗漏时扩展到该组页面。在 `context.hostReview` 记录页 ID、相关 flag、实际画面判断和采纳/补审决定；截图打开记录、哈希和检查数量只证明流程执行。画面上看不出影响的 finding 可以 `kept` 并写理由；裁决缺乏页面证据时交回补审。
- 修复目标是画面变好，不是 flag 变少：不为消掉线索而做 1–2px 的凑数挪动、塞装饰填空带、改 class 名绕开检测。修复后线索仍在但画面已解决的，由审查者裁决为 `dismissed`；flags 清零也不等于 clean，clean 依据真实画面裁决及主 Agent 的复核。
...
完成标准：主 Agent 对照本轮截图清单与线程成功看图工具记录，确认 plain/ruler 均有实际查看；缺项交回同一线程补看，在 `limits` 中保留未完成范围。覆盖确认后仍按上述规则复核采纳项和被驳回项；只读模式直接跳到第 5 步。
...
- 对齐收敛到同一条参考线；孤字、避头尾优先用宽度调整和语义断行解决，不靠缩小字号。字号、行距或宽度变化后，复看受影响文字的实际折行和相邻标签，尤其是句尾孤字与拆散的短语。
- 标点偏窄是复核线索；确认字体回退或错误字形后再改字体栈或字符，不从单个引号宽度推断使用了错误字体。
- 字数过多时，在用户给定的改稿范围内考虑压缩要点或拆页。用户要求保全已确认文字、或任务只允许改样式时，保留文字与页序，通过版式处理；不能消除的问题如实记录。
...
完成标准：按首轮相同方式核对本轮实际看图覆盖；审查者给出 clean，且主 Agent 对本轮采纳项和被驳回项的复核没有未解决分歧；或剩余项全部有明确阻塞原因。主 Agent 的修复说明不等于复验通过，只有审查者看过真实 after 才算 `fixed`。
