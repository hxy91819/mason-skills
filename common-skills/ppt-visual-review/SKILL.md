...
完成标准：审查者覆盖全部页，主 Agent 已按上述规则复核采纳项和被驳回项；只读模式直接跳到第 5 步。
...
完成标准：审查者给出 clean，且主 Agent 对本轮采纳项和被驳回项的复核没有未解决分歧；或剩余项全部有明确阻塞原因。主 Agent 的修复说明不等于复验通过，只有审查者看过真实 after 才算 `fixed`。
