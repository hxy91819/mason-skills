...
何时停止按 [SKILL.md 第 2 节](../SKILL.md#2-进程生命周期与数据保留)判断：开发、预览或等待验收期间持续复用环境，不随单轮任务收尾停止。确定不再需要后，项目 `bin/dev stop` 仅停止本环境进程与容器，默认保留数据和代理登记；服务停止后已认证请求出现 502 属于预期，不能据此终止其他环境。销毁环境时，按归属注销其配置和清单记录，校验后 reload。通用脚本模板不自动注册或注销路由，由 Agent 或管理员按本节执行。
...
- 验证启停隔离时使用两个可停止的测试环境，停止其中一个后另一个仍可访问；不为重复验收中断用户仍需使用的预览。
- 未登记子域被拒绝，业务端口不对外暴露。
