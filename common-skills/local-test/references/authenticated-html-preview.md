...
动态应用（前端、API、WebSocket）的子域名预览使用 [local-test 服务器配置](server-preview-setup.md)。两者可共用 Caddy 实例和账号库；动态应用通过独立认证子请求校验 Cookie，保留业务 Authorization，不直接套用本文删除 Authorization 的静态代理链路。项目服务生命周期不受本文 HTML 到期清理器管理。

Agent 发布生成的 HTML 时加载 `$dev-html-preview`，按其配置运行带文件锁的发布、续期和清理命令。新脚本与本文简易清理器不能同时管理同一目录，迁移方法由该 Skill 的部署与清理参考负责。
