...
Agent 发布生成的 HTML 时使用 [dev-html-preview skill](../common-skills/dev-html-preview/SKILL.md)，按其配置运行带文件锁的发布、续期和清理命令。新脚本与本文简易清理器不能同时管理同一目录，迁移方法见 [部署与清理](../common-skills/dev-html-preview/references/setup.md)。
